...
テスト方針全体の正本は `../../specs/development-workflow/README.md` とし、このドキュメントにはE2Eに固有の詳細を置く。シナリオ詳細は必要なものだけ読めるように同じディレクトリ配下へ分割する。
...
- [分類ルール](category-rules.md): 店名キーワード分類ルールの追加、PDF取込時の自動分類、無効化、削除。
- [監査ログ](audit-logs.md): フィルタ、一覧表示、ページ送り。
