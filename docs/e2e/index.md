...
フロントエンドの Integration Test は Vitest / React Testing Library / MSW による API mock 前提の画面結合テストとして扱い、ログイン、明細一覧、ダッシュボードの主要表示、APIエラー、再取得を高速に確認する。E2Eは実ブラウザ、実バックエンド、MySQL、Cookieを含む代表導線に集中し、細かいエラー分岐や表示再取得は Frontend IT へ寄せる。
