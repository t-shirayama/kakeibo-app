...
- 明細操作ユースケース、カテゴリ管理ユースケース、分類ルール管理ユースケースは分離する。
...
- 業務例外は `app/application/errors.py` の `ApplicationError` を基底にし、404などHTTPへ変換が必要な分類は例外自身の `status_code` で表す。API層でエラーメッセージ文字列を解析して分岐しない。
...
- 認証依存関係
- Swagger/OpenAPI公開
- application層の業務例外をHTTP例外と共通エラーレスポンスへ変換する責務
