...
- バックエンドの単体テストだけを確認する場合は `docker compose run --rm --no-deps backend python -m pytest tests/unit` を使う。単体テストのカバレッジを確認する場合は `docker compose run --rm --no-deps backend python -m pytest tests/unit --cov=app --cov-report=term-missing` を使い、`backend/pyproject.toml` の coverage 設定で80%以上を必須にする。単体テストはMySQL起動に依存させない。
...
- フロントエンドの単体テストだけを確認する場合は `docker compose run --rm --no-deps frontend npm run test:unit`、単体テストのカバレッジを確認する場合は `docker compose run --rm --no-deps frontend npm run test:unit:coverage`、Integration Test だけを確認する場合は `docker compose run --rm --no-deps frontend npm run test:integration` を使う。いずれもバックエンドやMySQL起動に依存させない。依存追加直後など、`frontend-node-modules` ボリュームが古い場合は `docker compose run --rm --no-deps frontend npm install` で lockfile を反映してから実行する。
