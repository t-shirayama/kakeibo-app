...
- [ ] 実データを用いてPDF取込機能の精度向上、検証、リファクタリングを行う
  ...
  - 対象: `docs/specs/pdf-import.md`、`backend/app/application/importing/`、`backend/app/infrastructure/parsers/`、`backend/tests/`、必要に応じて `frontend/e2e/upload.spec.ts`
  - 対応: 実データをもとに抽出失敗や誤分類の傾向を整理し、PDF取込処理の判定ロジックと責務分割を見直したうえで、再現用fixtureと検証テストを追加する
  - 完了条件: 実データ由来の代表パターンがfixture化され、精度改善内容と制約が `docs/specs/pdf-import.md` に反映され、関連するバックエンドテストと必要なE2Eで回帰確認できる
